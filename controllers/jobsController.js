...
exports.jobStats = catchAsyncErrors(async (req, res, next) => {
  ...
  if (stats.length === 0) {
    return next(
      new ErrorHandler(`No stats were found for - ${req.params.topic}`)
    );
  }
  res.status(200).json({
    success: true,
...
// Apply to job using resume => api/v1/job/apply/id
exports.applyJob = catchAsyncErrors(async (req, res, next) => {
  ...
  if (job.lastDate < new Date(Date.now())) {
    return next(
      new ErrorHandler("You cannot apply to this job. Date is passed.", 400)
    );
  }
